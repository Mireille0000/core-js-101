...
function findFirstSingleChar(str) {
  return [...str].filter((item) => !(str.split(item).length - 2))[0] || null;
}
...
function reverseInteger(num) {
  ...
  for (let i = 0; i < numArr.length; i += 1) {
    result += numArr[numArr.length - 1 - i];
  }
...
function isCreditCardNumber(ccn) {
  const ccnNumbersArr = ccn.toString().split('').map((item) => +item);

  for (let i = ccnNumbersArr.length - 2; i >= 0; i -= 2) {
    let checkingValue = ccnNumbersArr[i];
    checkingValue *= 2;

    if (checkingValue > 9) {
      checkingValue = (checkingValue % 10) + 1;
    }

    ccnNumbersArr[i] = checkingValue;
  }

  let count = 0;

  for (let i = 0; i < ccnNumbersArr.length; i += 1) {
    count += ccnNumbersArr[i];
  }
...
function getDigitalRoot(num) {
  ...
  const mapStr = numToString.map((item) => parseInt(item, 10));
  const mapReduced = mapStr.reduce((acc, item) => {
    let accum = acc;
    accum += item;
    return accum;
  }, 0);

  if (mapReduced.toString().split('').length >= 2) {
    const newP = mapReduced.toString().split('');
    const newParsedMap = newP.map((item) => parseInt(item, 10));
    return newParsedMap.reduce((acc, item) => {
      let accum = acc;
      accum += item;
      return accum;
    }, 0);
